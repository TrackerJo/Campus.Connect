...
function OpportunityTile({ opportunity, accountType, studentData, onEdit, onRemove, onApply, onRSVP,onViewAttendees }: OpportunityTileProps) {
    ...
    function oneTimeVolunteerOpp(oneTimeOpp: OneTimeVolunteer) {
        return (
            <div className="opportunity-tile">
                ...
                {accountType === "student" && <br />}
                {accountType === "student" ? isLoadingRSVP ? <div className="loader"></div> : <button className="ActionBtn" onClick={async () => {
                    //Apply to opportunity
                    if (oneTimeOpp.applicationLink === "") {
                ...
                }}>RSVP</button> : isLoadingRSVP ? <div className="loader"></div> : <button className="ActionBtn" onClick={async () => {
                    //Apply to opportunity
                    if (oneTimeOpp.applicationLink === "") {
                        setIsLoadingRSVP(true)
                        await onViewAttendees()
                        setIsLoadingRSVP(false)
                        return
                    }
                    window.open(oneTimeOpp.applicationLink, "_blank");
    ...
    function recurringVolunteerOpp(recurringOpp: RecurringVolunteer) {
        ...
        return (
            <div className="opportunity-tile">
                ...
                {accountType === "student" && isLoadingRSVP ? <div className="loader"></div> : <button className="ActionBtn" onClick={async () => {
                    //Apply to opportunity
                    if (recurringOpp.applicationLink === "") {
                        setIsLoadingRSVP(true)
                        await onRSVP()
                        setIsLoadingRSVP(false)
                        return
