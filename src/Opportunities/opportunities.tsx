...
import { Company, Opportunity, OpportunityType, ShiftType, Location, CompanyType, Job, OneTimeVolunteer, EventDate, RecurringVolunteer, StudentData, ActivityMember } from '../constants'
...
import AttendeesDialog from '../components/Attendees_Dialog'
...
function App() {
    ...
    const attendeesDialogRef = useRef<HTMLDialogElement>(null)
    const [attendees, setAttendees] = useState<ActivityMember[]>([])
            ...
            getUserData().then((studentData) => {
                ...
                getOpportunities().then((opportunities) => {
                    ...
                    setOpportunities(opportunities.filter((opp) => {
                        if (opp.applicants.find((applicant) => applicant.userId == student!.uid) && opp.type == OpportunityType.Job) {
                            return false
                        }
                        return true
                    }))
        ...
        return (
            <>
                <div className='createOpportunity'>
                    ...
                    <div className='FormRow'>
                        <label htmlFor="">Application Link {opportunityType != OpportunityType.Job ? "(Optional)" : ""}: </label>
                    ...
                    {isCreatingOpportunity ? <div className='loader'></div> : <button className='ActionBtn' onClick={async () => {
                        if (position == "" || description == "" || location == null) {
                            alert("Please fill out all fields")
                            return
                        }
                        if (opportunityType == OpportunityType.Job && applicationLink == "") {
                            alert("Please fill out all fields")
                            return
                        }
    ...
    return (
        <>
                ...
                {createOpportunity || isEditingOpportunity ? createOpportunityTile() : <><div className='opportunities'>
                    {opportunities.map((opportunity) => {
                        return <OpportunityTile opportunity={opportunity} accountType={accountType} studentData={studentData}
                            onViewAttendees={() => {
                                setAttendees(opportunity.applicants)
                                if (opportunity instanceof OneTimeVolunteer) {
                                    setAttendees(opportunity.volunteers)

                                }
                                attendeesDialogRef.current?.showModal()
                            }
                            }
                            onApply={() => {
                            ...
                            onRSVP={async () => {
                                //remove opportunity from list
                                const newOpportunity = Opportunity.fromMap(opportunity.toMap())
                                newOpportunity.applicants.push(studentData!.toActivityMember())
                                if (newOpportunity instanceof OneTimeVolunteer) {
                                    newOpportunity.volunteers.push(studentData!.toActivityMember())
                                }
                                if (newOpportunity instanceof RecurringVolunteer) {
                                    newOpportunity.volunteers.push(studentData!.toActivityMember())
                                }
                                newOpportunity.lastUpdated = Date.now()
                                await updateOpportunity(newOpportunity, true)
            ...
            <AttendeesDialog attendees={attendees} dialogRef={attendeesDialogRef} close={() => {
                attendeesDialogRef.current?.close()
            }} />
        </>
    )
}
